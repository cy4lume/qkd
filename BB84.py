...
N = 10
TOKEN = "" # your IBM token string here
...
class B(Person):
    def __init__(self, backend, sampler):
        super().__init__()
        self._backend: ibm_backend.IBMBackEnd = backend
    ...
    def measure(self, message: list[QuantumCircuit]) -> None:
        ...
        transpiled_circuits = [transpile(circ, backend=self._backend, optimization_level=0) for circ in message]

        for i in trange(self.n):
            result = self._sampler.run([transpiled_circuits[i]], shots=1).result()
            measured_bit = int(result[0].data.c.array[0])
...
def init(argv) -> tuple[logging.Logger, ibm_backend.IBMBackend, Sampler]:
    """
    Parse CLI arguments and initialize IBM Quantum service.
    """
    global TOKEN
    ...
    if TOKEN == None:
        logger.warning("TOKEN is not set. Please set your IBM quantum token.")
        sys.exit(-1)
    ...
    if sampler == None:
        logger.error("Sampler Initialization Failed")
        sys.exit(-1)
    else:
        logger.debug("Initialization Succeeded")

    return logger, backend, sampler


def main(argv):
    logger, backend, sampler = init(argv)

    alice = A()
    bob = B(backend, sampler)
    eve = B(backend, sampler)  # the eavesdropper
